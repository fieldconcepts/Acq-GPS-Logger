...
class MirfFile:
    ...
    A class that parses MIRF (v7) rcd files.
    ...
    def save_file(self,filename):
        ...
        if self.channel_count < 1:
            raise Exception("There are no channels to save!")
...
#             print("{:02d}: {}".format(i,d))
        ...
        self.time = datetime.datetime(fields[9],fields[10],fields[11],fields[12],fields[13],fields[14],fields[48],tzinfo=datetime.timezone(datetime.timedelta(seconds=fields[15])))
    ...
    def get_finish_timestamp(self):
        return self.time + \
            datetime.timedelta(microseconds=self.sample_period*self.channels[0].N)
            
    def get_sample_offset(self, timestamp):
        
        if isinstance(timestamp, datetime.datetime):
            if timestamp < self.time: raise IndexError("Timestamp is before dataset records.")
            if timestamp > self.get_finish_timestamp(): raise IndexError("Timestamp is after dataset records.")
            
            tdiff = (timestamp - self.time)/datetime.timedelta(microseconds=self.sample_period)
            return tdiff
        
        raise ValueError("The timestamp must be a valid time format.")
    ...
    class _Channel():
        ...
        def __init__(self,cid,raw_header=None):
            ...
            if raw_header:
                ...
                self.legacy_owner = 0
                self.__descriptor = 0
                self.__format_code = 4
                self.N = 0
                self.pointer_us = 0
                self.__owner = 0
                self.rcx = 0
                self.rcy = 0
                self.vertical_depth = 0
                self.depth_offset = 0
                self.hsi = 0.
                self.reserved = 0
                self.sensor_scaling = 1.0
                self.dc_offset = 0.
                self.scaling_factor = 1.0
                self.max_magnitude = 0.
        
        descriptor_values = ['Unknown data','VZ','HX','HY','DH','HYDRO','PILOT','TB',
                  'GF','RM','BP','PPS','VSIM','RSIM','NGEO','FGEO']
        
        format_values = ['DAQ IFP', 'Multilock IFP', 'Geochain IFP',
                  '32-bit Int','32-bit Float',
                  '24-bit Int', '16-bit Int', '16-bit Delta IFP']
        
        owner_values = ['AUX','REF','NONE']
        
        @property
        def descriptor(self):
            "The type of data that is stored in the channel."

            return MirfFile._Channel.descriptor_values[self.__descriptor]
        
        @descriptor.setter
        def descriptor(self, x):
            if isinstance(x, int):
                if(x < 0 or x >= len(MirfFile._Channel.descriptor_values)):
                    raise ValueError("The integer {} does not represent a valid MIRF channel descriptor.".format(x))
                self.descriptor = MirfFile._Channel.descriptor_values[x]
            elif isinstance(x, str):
                try:
                    self.__descriptor = MirfFile._Channel.descriptor_values.index(x)
                except ValueError:
                    raise ValueError("'{}' is not a valid MIRF channel descriptor.".format(x))
            else:
                raise ValueError("Cannot set the channel descriptor with value \"{}\"".format(x))
        
        @property
        def format_code(self):
            "The data format of the channel stream."
            
            return MirfFile._Channel.format_values[self.__format_code]
        ...
        def owner(self):
            ...
            if self.__owner < 1:
                return MirfFile._Channel.owner_values[self.__owner+2]
            return self.__owner
